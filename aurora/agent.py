    ...
    async def _run_provider(
        ...
        async with httpx.AsyncClient(timeout=90) as client:
            for _ in range(max_steps):
                body = {"model": provider["model"], "messages": [{"role": "system", "content": "You are Kim, a concise practical technical personal agent. Inspect first. Never claim an action happened unless its tool succeeded. Any data change, message, device control, typing, file edit, or command requires user approval; ask for approval instead of bypassing it. For on-screen/desktop work, verify each step before moving on: after an action, run computer_action see (or describe) again to check the screen, then either proceed to the next step or retry with a corrected action. Keep taking steps until the user's goal is finished; only stop when you can show a verified result. Do not narrate a running sequence ('opening X now', 'trying Y now', 'attempting Z now'): fallback resolution happens inside the tools, so report only the final outcome once it is known."}] + history, "tools": self._tools(), "tool_choice": "auto", "temperature": 0.2}
