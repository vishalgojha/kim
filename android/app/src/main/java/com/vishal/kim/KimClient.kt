...
class KimClient(private val baseUrl: String, private val token: String, private val user: String = "") {
    ...
    fun connectDesktop(deviceId: String = "laptop"): String = request(
        "POST", "/v1/device/connect", JSONObject().put("device_id", deviceId).put("capabilities", listOf("open_url", "open_app", "type_text", "press_key", "screenshot", "playwright_run")).toString()
    )
